  ...
  return (
    <SiteShell>
      ...
      <section className="mx-auto max-w-7xl px-6 py-10 lg:px-8">
        <div className="rounded-[2rem] border border-[#0A2540]/10 bg-white p-8 shadow-[0_18px_60px_rgba(10,37,64,0.08)]">
          {courses.length > 0 ? (
            <div className="grid gap-6 lg:grid-cols-2">
              {courses.map((course) => {
                const embedUrl = getYouTubeEmbedUrl(course.url);
                return (
                  <div key={course.id} className="overflow-hidden rounded-[1.5rem] border border-[#0A2540]/10 bg-[#FAFAFA] shadow-[0_20px_60px_rgba(10,37,64,0.06)]">
                    {embedUrl ? (
                      <iframe
                        src={embedUrl}
                        title={course.title}
